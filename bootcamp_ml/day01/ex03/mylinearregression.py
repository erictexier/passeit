...
class MyLinearRegression():
    ...
    def insert_ones(x):
        ...
        return np.concatenate((xp,x), axis=1)
    ...
    def fit_(self, X, Y, alpha = 0.01, n_cycle=2000):
        ...
        X2 = self.insert_ones(X)
        for i in range(n_cycle):
            theta = self.theta - (alpha * self.vec_gradient(X2, Y, self.theta))
            self.theta = theta
        return (self.theta)
...
print(x.cost_(X1,Y1))
theta2 = x.fit_(X1, Y1, alpha = 0.01, n_cycle=500)
print(theta2)
